...
public class TeamFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final Spinner playerCountSpinner = view.findViewById(R.id.player_count_spinner);
        final String[] playerCountOptions = new String[(MAX_PLAYER_COUNT - MIN_PLAYER_COUNT) + 1];

        for (int count = MIN_PLAYER_COUNT; count <= MAX_PLAYER_COUNT; count++) {
            playerCountOptions[count - MIN_PLAYER_COUNT] = count + " Players";
        }

        ArrayAdapter<String> playerCountOptionAdapter = new ArrayAdapter<>(getActivity(),
                android.R.layout.simple_spinner_item,
                playerCountOptions);
        playerCountOptionAdapter
                .setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        playerCountSpinner.setAdapter(playerCountOptionAdapter);

        final RecyclerView recyclerView = view.findViewById(R.id.recycler_view);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        final Button createTeamButton = view.findViewById(R.id.create_team_button);
        createTeamButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                List<Player> players = new ArrayList<>();
                for (int i = 0; i < playerCountSpinner.getSelectedItemPosition() + MIN_PLAYER_COUNT; i++) {
                    players.add(new Player());
                }

                playerCountSpinner.setVisibility(View.GONE);
                createTeamButton.setVisibility(View.GONE);

                recyclerView.setAdapter(new PlayerAdapter(getActivity(), players));
            }
        });
